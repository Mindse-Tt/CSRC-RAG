...
> Owner: EngineeringAgent（前后端工程链路）  
> Coordinator 合稿 input · 与 L0–L7 各策略对齐（01 拒答 / 03 query 改写 / 07 response / 10 模型选型）  
...
if "%1"=="--legacy" (
  ...
  "C:\Users\mindxu\AppData\Local\Programs\Python\Python312\python.exe" scripts\run_demo_server.py
  goto :end
)
...
"C:\Users\mindxu\AppData\Local\Programs\Python\Python312\python.exe" -m uvicorn csrc_rag.api.server_v2:app ^
...
> 合稿时请 Coordinator 按需裁剪，本文档 ≈ 1600 字正文 + 多张表 / 图，供团队内部对齐；不入最终论文。
