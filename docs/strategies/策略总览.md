...
| L | 99 Leader | 本文 + `一页纸决策清单.md` | 编排 11 策略 + 裁决 11 条冲突 + 风险 Top-12 | — | 所有 | 组员 |
...
⚠️ **重要修正**：`00-shared-context.md` 和 `99-leader.md` 均写"本机无 GPU / 需 Colab"，**已过时**。实际以本机 RTX 2060 SUPER 8GB 为主训，Colab/Kaggle 作为 OOM 兜底。
...
| **Qwen2.5-1.5B QLoRA 主训** | Qwen2.5-1.5B-Instruct | 5,500 条 8 类 QA（见下） | **本机 RTX 2060S** 或 Colab T4 兜底 | 3–5h | 王怡菲 |
| LoRA Ablation V1/V2/V3 | 同上 | V1=A+B+C (4,000) / V2=+D+E+F (5,000) / V3=+G+H (5,500) | 同上 | 各 ~3h | 王怡菲 |
...
**rag_qa_train.jsonl 5,500 条分布**（见 08 §3）：A 案例 1,800 / B 法条 1,200 / C 处罚 1,000 / D 趋势 400 / E 拒答 300 / F 证据不足 300 / G 多轮 200 / H 反幻觉 300。
...
⚠️ **新增 C12（合稿时发现）**：08 QLoRA 用 `bnb_4bit_compute_dtype=bfloat16`，但用户 2060S 是 Turing 架构**不支持 bf16** → 裁决：**全链路改 fp16**，`08` 文档 bf16 字样由 Coordinator 在下一轮 patch 时替换。
...
gantt
    ...
    section 准备期 D0-D2
    策略文档合稿(Coord)                   :done, m0, 2026-04-22, 1d
...
| R10 | 策略文档 bf16/Colab/无 GPU 口径过期 | 中×低 | Coord 下一轮 patch 统一替换；本文档已记录修正 | Coordinator |
...
> 原始 CNRDS 证监会处罚信息表共 14,740 条行记录（开题 14,743 为表头计数偏差，修订为 14,740）。以 **EventID** 为最小事件单位聚合得到 **4,233 个事件级文档**；以当事人为粒度保留 **14,740 条分类样本**。"8,000 有效"为开题阶段估算，实际采用 4,233（检索语料）+ 14,740（监督样本）。事件级再切分为 29,314 个 chunk 供细粒度检索。切分规则：EventID+时间双切，Train 1994-2021 / Val 2022-2023 / Test 2024-2025，严格防泄漏。

---

> **Coordinator 结语**：技术栈已锁死、冲突已裁决、分工已落名、甘特已按日编排。下一步进入执行期：① 王怡菲 D1 测通 RTX 2060S fp16 QLoRA smoke（1 个 batch 跑通即可）；② Coord 下一轮把 08 文档 bf16 字样 patch 成 fp16；③ 全员按 §8 分工表动手。
