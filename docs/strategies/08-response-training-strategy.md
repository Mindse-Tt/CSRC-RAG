# 08 · 回复模型训练策略（StrategyAgent-H · QLoRA）

> 负责人：StrategyAgent-H
> 上游依赖：`00-agent-team-shared-context.md`、`docs/微调方案.md`、检索层产物（`event_corpus.jsonl` / `party_samples.jsonl`）
...
| `save_steps` | 200 | 训练总步数 ≈ 1030 步（5500×3÷16），存 5 个 checkpoint |
...
## 3. 数据构造：5500 条 · 8 类（A/B/C/D/E/F/G/H）
...
总计 **5500 条**，train:val:test = 5000:300:200（val/test 按 EventID 切分，**不泄漏**）。
...
> **注**：不由 StrategyAgent-H 直接改 `responder.py`（按 shared-context 规定"不动别人文件"），改动方案写进交接单交给 Coordinator。
...
## 8. 训练时长预估（5500 条 × 3 epoch · 有效 batch=16）
...
flowchart TD
    ...
    C -->|G 多轮| D5[200]
    C -->|H 反幻觉| D6[300]
    D1 & D2 & D3 & D4 & D5 & D6 --> E[rag_qa_train.jsonl 5500]
...
> 文档版本：v1.0 · 2026-04-22 · StrategyAgent-H
