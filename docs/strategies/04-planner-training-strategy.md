# 04 · Planner 规划器训练策略（StrategyAgent-D）

> Owner: StrategyAgent-D · 上游：L0 预处理 · 下游：L2 Query 改写 + L3 检索 + L5 生成
>
> 参见共享上下文：[00-agent-team-shared-context.md](./00-agent-team-shared-context.md)
...
2. **人工抽检**：每类抽 30 条（6%）由 Coordinator 复核标签；
...
- [ ] （后续）`src/csrc_rag/orchestration/planner.py` 输出 `QueryPlan`（由 Coordinator 合稿阶段接入）
