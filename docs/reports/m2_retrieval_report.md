...
**日期**: 2026-04-22
**执行**: ExecutionAgent-M2Retrieval

---

## 1. 工作范围

本次升级按照 `docs/strategies/05-retrieval-strategy.md` §3.6 提出的五大根因修复其中的 R1（Dense encoder 从英文 MiniLM → 中文 bge-small-zh），并叠加 `06-reranking-strategy.md` 中的 L3 尾部 cross-encoder 精排。R2（query_plan 硬过滤）、R3（jieba tokenizer）、R4（top-50 硬截断）由后续 StrategyAgent-F/G 与 M3 负责。
