...
- **Agent**: ExecutionAgent-M1d (QLoRA smoke test)
- **日期**: 2026-04-22
...
⚠️ **必须 override `configs/qlora_config.json` 的 `bf16: true` 为 `false` + `fp16: true`**（该配置原为 A100/H100 场景，Turing 不支持 bf16）。smoke 未改该文件，符合"禁止改正式配置"约定，正式训练前由 StrategyAgent 或 ExecutionAgent 显式设置。
...
5. **下一步**：StrategyAgent 或正式 Execution 阶段生成 `data/train/rag_qa_train.jsonl` 真实语料后，即可在 `configs/qlora_config.json` 上改 `bf16→fp16` 启动正式训练
