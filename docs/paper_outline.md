...
1.2 问题定义
- 幻觉问题：零样本场景下编造不存在的法条、金额（具体数据：裸模型20%幻觉率）
...
- 贡献3：在<1B、8GB GPU约束下实现3.3%幻觉率，接近GPT-4水平
- 贡献4：Multi-Agent Team开发模式，策略-执行分离
...
```
3.1 数据集构建
- 数据来源：CNRDS证监会处罚信息表（14,740行×24列）
- 数据处理流水线：
...
4.4 训练过程分析
- 【插入 Figure 5: loss曲线】
- 收敛情况：loss 2.52→0.70, 274 steps, 52 min
- 过拟合检测：eval_loss稳定
...
实现了：幻觉数字率3.3%（相对下降83%）、格式合规率76.7%、EventID命中率20%。
...
| 成员 | 贡献 |
|------|------|
| 许浩财 | 系统架构设计、Agent Team搭建、QLoRA训练、评测体系设计、论文主笔 |
| 贾彤 | [填写] |
| 戴一鑫 | [填写] |
| 张彦扬 | [填写] |
| 王怡菲 | [填写] |
...
| AI工具 | 用途 | 具体环节 |
|--------|------|---------|
| Claude Code (Anthropic) | Multi-Agent编排、代码生成、文档撰写 | 系统架构设计、评测脚本编写、策略文档生成 |
| GPT-5.2 (OpenAI) | Agent Team Runner 中的生成引擎 | 训练数据QA对构造、查询改写 |
| [其他] | [填写] | [填写] |
