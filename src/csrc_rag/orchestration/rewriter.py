"""L2 Query Rewriter — 三层流程：共指消解 + 同义词扩展 + 槽位抽取。

Owner: StrategyAgent-C
Non-invasive stub — 不依赖 L3/L5，兜底降级为 raw_query。

Pipeline:
    raw_query + history
      -> L2a coref resolution (rule first, LLM fallback)
      -> L2b synonym expansion (dict-based)
      -> L2c slot extraction (via slot_filler)
      -> RewriteOutput
"""
