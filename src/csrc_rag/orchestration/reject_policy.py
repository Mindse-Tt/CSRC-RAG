"""Reject policy -- multi-level fallback for out-of-scope / low-confidence queries.

This module implements the rejection + fallback decision layer described in
``docs/strategies/01-reject-strategy.md``. It aggregates signals from the
existing ``intent_model`` (ML classifier) and ``topic_guard`` (hard keyword
rules) into a single ``IntentDecision`` object, and provides hooks for
L3 (retrieval empty), L5 (low generation confidence) and L7 (citation check
failed) downstream fallbacks.

Intentionally does NOT modify ``intents.py``, ``intent_model.py`` or
``topic_guard.py`` -- this file only consumes their public APIs.
"""
...
INTENT_CONFIDENCE_FLOOR: float = 0.60   # below → fall back to topic_guard vote
RETRIEVAL_MIN_HITS: int = 2             # L3 minimum acceptable hits
RETRIEVAL_MIN_SCORE: float = 0.25       # L3 minimum top rerank score
GENERATION_MIN_LOGPROB: float = -1.5    # L5 minimum mean token logprob
CITATION_MIN_HIT_RATE: float = 0.70     # L7 minimum evidence-citation recall
