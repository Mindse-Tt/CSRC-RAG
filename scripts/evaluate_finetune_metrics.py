...
如果没有 predictions 文件，则对 reference 自身做分析展示指标定义和计算方式。
"""
...
def extract_entities(text: str) -> dict[str, set[str]]:
    """Extract domain entities from structured answer."""
...
def compute_entity_f1(
    ...
    """Compute micro-averaged Precision, Recall, F1 across all entity types."""
    tp = 0
    fp = 0
    fn = 0
...
def check_format_compliance(text: str) -> bool:
    """Check if output follows the required citation format."""
...
def check_hallucinated_numbers(pred_text: str, ref_text: str, evidence_text: str) -> tuple[int, int]:
    """Count hallucinated vs total numbers in prediction.

    A number is hallucinated if it appears in prediction but not in reference or evidence.
    """
    # Extract all numbers from texts
    ...
    # Filter trivial numbers (single digits, common patterns)
...
def evaluate(
    ...
    for pred, ref in zip(predictions, references):
        ...
        # 4. Task Accuracy (strict: ALL reference EventIDs must appear)
...
def evaluate_reference_ceiling(references: list[dict]) -> MetricsResult:
    """Evaluate reference answers against themselves (ceiling = 100%)."""
    return evaluate(references, references)


def simulate_g0_baseline(references: list[dict]) -> MetricsResult:
    """Simulate G0 baseline metrics (model without RAG/LoRA)."""
    # G0 produces generic refusals or hallucinated content
    g0_predictions = []
