...
def fig_architecture() -> None:
    ...
    layers = [
        ...
        (3.5, "L5  Qwen2.5-0.5B + QLoRA · 受证据约束生成", C_RED, "white"),
...
def fig_g0_g3() -> None:
    ...
    ax.set_ylabel("Rate")
    ax.set_ylim(0, 1.0)
    ax.set_title("Figure 2 · 四组生成对照 (n = 30, gold_130 分层抽样)", pad=8)
...
# ---------------------------------------------------------------------------
# Figure 5 — QLoRA training loss curves
# ---------------------------------------------------------------------------
def fig_loss() -> None:
    ...
    ax.set_title("Figure 5 · QLoRA 训练收敛曲线 (Qwen2.5-0.5B, 2 epoch, 274 steps)", pad=8)
...
def fig_corpus() -> None:
    ...
    fig.suptitle("Figure 6 · 知识库分布 (n = 4,233 events from CNRDS 2000-2025)",
                 fontsize=11, y=1.02)
    fig.tight_layout()
