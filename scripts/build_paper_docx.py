...
def section_cover(doc: Document) -> None:
    ...
    bold_run(
        doc,
        "研究方法。",
        "本研究构建了一个端到端的七层检索增强问答流水线,知识库基于 CNRDS 14 740 条原始"
...
def section_intro(doc: Document) -> None:
    ...
    para(
        ...
        "上升趋势。根据本研究收集的中国研究数据服务平台(CNRDS)证监会处罚信息表,"
    ...
    para(
        ...
        "层面的工作:(1) 基于 CNRDS 原始数据构建事件级检索知识库与多粒度的人工标注金标;"
...
def section_data(doc: Document) -> None:
    ...
    para(
        doc,
        "原始数据来源于中国研究数据服务平台(CNRDS)「证监会处罚信息表」,涵盖"
...
def section_contributions(doc: Document) -> None:
    ...
    para(
        doc,
        "本研究的代码仓库托管于 https://github.com/Mindse-Tt/Deeplearning-Rag-Test(答辩前"
        "对 reviewer 开放访问),tag v1.0-track-b-final 对应本论文所用的确定版本。"
        "所有训练数据、LoRA adapter 权重以及评测集均可通过 Git 仓库下载,或通过 "
        "scripts/build_*.py 系列脚本从原始 CNRDS 数据重新构建。",
    )
